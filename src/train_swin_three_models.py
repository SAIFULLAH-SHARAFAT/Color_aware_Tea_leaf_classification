...
class SwinRGBHSV(nn.Module):
    def __init__(
        ...
        # FAIRNESS CONTROL: use the same classifier head for RGB and HSV
        # variants.  The previous script used a linear RGB head but a two-layer
        # MLP for HSV variants, which confounded small HSV gains with head
        # capacity.  All fresh R0--R3 runs now share this exact head.
    ...
    def initialization_scale_diagnostics(
        ...
        """Measure RGB/HSV feature scale before any optimizer update.

        The model must be called in eval mode by the caller.  This routine does
        not modify parameters, BatchNorm running statistics, or dropout state.
        It reports the *ungated* projected HSV scale separately from the
        conservative sigmoid gate so an under-scaled projection is not confused
        with the intended gate initialization.
        """
        if not self.use_hsv_branch:
...
class Trainer:
    ...
    def _write_initialization_scale_diagnostics(self) -> None:
        ...
        Uses eval mode + no_grad, so the diagnostic does not update BatchNorm,
        consume dropout masks, or modify model weights.  It is intentionally
        measured before optimizer construction and before any training epoch.
        """
        base_model = self.model.module if isinstance(self.model, nn.DataParallel) else self.model
    ...
    def _build_loaders(self) -> Tuple[DataLoader, DataLoader, DataLoader]:
        ...
        # All R0-R3 variants use the SAME hue-jitter setting.  For the paper
        # experiments we set hue_jitter=0.0 for every variant in experiments.yaml.
        # Do not silently change augmentation as a function of architecture.
    ...
    def fit(self):
        ...
        # Write the initialization diagnostic only for a genuinely fresh run.
        # On resume, never overwrite the original pre-optimization measurement.
        ...
        # Save stable image identifiers so paired bootstrap aligns the SAME
        # test images across independent training harnesses, rather than merely
        # assuming identical dataset order from the label vector.
