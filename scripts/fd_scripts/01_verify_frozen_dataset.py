#!/usr/bin/env python3
"""Verify the frozen 7,714-instance dataset before any experiment is launched."""
...
def main():
    ...
    df=pd.read_csv(mp)
    required={"split","class_name","filename","md5","canonical_id","is_derived"}
    miss=required-set(df.columns)
    if miss: raise SystemExit(f"Manifest missing columns: {sorted(miss)}")
    # Recreate builder fingerprint exactly: sorted stable columns, CSV without index.
    ...
    sha=hashlib.sha256(canon.to_csv(index=False).encode()).hexdigest()
