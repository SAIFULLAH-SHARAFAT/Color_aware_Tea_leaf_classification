...
Emits:
    results/tables/per_run.csv          one row per (experiment, seed)
    results/tables/summary.csv          mean +/- sd over seeds
    results/tables/per_class_f1.csv     per-class F1, mean +/- sd
    results/tables/main_results.tex     main comparison table
    results/tables/per_class_f1.tex     per-class table
...
def matrix():
    import yaml
    return yaml.safe_load(open(REPO/"configs/experiments.yaml"))
...
def load_labels(config: dict) -> dict:
    try:
        ...
        return {k: v for k, v in cfg["experiments"].items()}
    except Exception:
        return {}
...
def collect(runs_dir: Path) -> pd.DataFrame:
    ...
    for run in sorted(runs_dir.iterdir()):
        ...
        row = {
            ...
            "run": run.name,
            "seed": c.get("seed"),
            "arch": c.get("arch", c.get("model", "")),
            "use_hsv": c.get("use_hsv", False),
            "lr": c.get("lr"),
...
def per_class(df: pd.DataFrame) -> Optional[pd.DataFrame]:
    ...
    for c in cols:
        name = c.split("::", 1)[1]
        out[name] = [f"{m:.4f} ± {s:.4f}" if not np.isnan(s) else f"{m:.4f}"
...
def latex_main(summary: pd.DataFrame, labels: dict, out: Path) -> None:
    lines = [
        "% generated by scripts/06_aggregate_results.py -- do not edit by hand",
        "\\begin{table}[t]",
        "\\centering",
        "\\caption{Test-set performance under the audited TeaLeafBD protocol. "
        "Values are mean $\\pm$ standard deviation over independent seeds. "
        "R0--R3 and the token-level controls use different training recipes. "
        "Inferential comparisons are made only within each predeclared family; "
        "cross-family values are descriptive only.}",
    ...
    for _, r in summary.iterrows():
        label = labels.get(r["experiment"], {}).get("label", r["experiment"])
        lines.append(
            f"{r['experiment']} & {label} & {int(r['n_seeds'])} & "
...
def latex_per_class(pc: pd.DataFrame, labels: dict, out: Path) -> None:
    ...
    lines = [
        ...
        "\\caption{Per-class F1, mean $\\pm$ standard deviation over seeds.}",
...
def main() -> None:
    ...
    for fam, name in [("token","token_primary_results.tex"),("hsv","hsv_complementary_results.tex")]:
        ids={k for k,v in labels.items() if v.get("family")==fam}
        sf=summary[summary["experiment"].isin(ids)].copy()
        if len(sf):
            latex_main(sf, labels, out/name)
    ...
    if pc is not None:
        ...
        latex_per_class(pc, labels, out / "per_class_f1.tex")
