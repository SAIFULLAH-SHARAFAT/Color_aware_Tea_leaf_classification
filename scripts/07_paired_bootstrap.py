...
Resampling is unstratified, so minority classes vary in representation across
replicates and the Macro-F1 intervals are conservative relative to a stratified
scheme. This is stated in the manuscript because it supports, rather than
weakens, a cautious reading of small differences.
...
across-seed standard deviations in results/tables/summary.csv.

USAGE
-----
    # one comparison
    python scripts/07_paired_bootstrap.py --a results/runs/TLA_s42 \\
                                          --b results/runs/C1_s42
...
def load_run(run_dir: Path) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if not ids_path.exists():
        raise SystemExit(
            f"{ids_path} missing. Fresh revision runs must save image IDs; "
...
def compare_experiment(runs_dir: Path, exp: str, baseline: str,
    ...
    for s in shared:
        ...
        print(f"  seed {s}: delta {r['observed_delta']:+.4f}  "
              ...
              f"P(delta>0) {r['p_delta_gt_0']:.3f}{star}")
    return out
...
def main() -> None:
    ...
    ap.add_argument("--runs-dir", default=str(REPO / "results" / "fresh" / "runs"))
    ap.add_argument("--out", default=str(REPO / "results" / "fresh" / "tables" / "bootstrap.csv"))
    ...
    if args.a and args.b:
        ...
        try:
            import yaml
            cfg = yaml.safe_load(open(REPO / "configs" / "experiments.yaml"))
            planned = cfg.get("paired_comparisons", [])
        except Exception:
            planned = []
    ...
    if "experiment" in df.columns:
        ...
        summ = pd.DataFrame({
            ...
            "seeds_ci_excludes_0": g["ci_excludes_zero"].sum(),
        }).reset_index()
